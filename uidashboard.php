...
define('SELF_BACKUP_DIR',   FIRMWARE_DIR . 'dashboard_backup/');
define('DASH_VER_FILE',     FIRMWARE_DIR . 'dashboard_version.json');
...
// ── Helper: dashboard version ─────────────────────────────────────────────
function getDashVersion() {
    if (!file_exists(DASH_VER_FILE)) return [
        'version'   => 'v1.0.0',
        'updated_at'=> date('Y-m-d H:i:s', filemtime(SELF_FILE)),
        'note'      => 'Versi awal',
    ];
    return json_decode(file_get_contents(DASH_VER_FILE), true)
        ?? ['version' => '-', 'updated_at' => '-', 'note' => ''];
}

// ── Helper: list backup ───────────────────────────────────────────────────
function getBackupList() {
    if (!is_dir(SELF_BACKUP_DIR)) return [];
    $files = glob(SELF_BACKUP_DIR . '*.php');
    if (!$files) return [];
    usort($files, fn($a, $b) => filemtime($b) - filemtime($a));
    return array_slice($files, 0, 10); // max 10 backup
}

// ── Helper: bersihkan backup lama (simpan 5 terbaru) ─────────────────────
function pruneBackups() {
    $files = getBackupList();
    foreach (array_slice($files, 5) as $old) @unlink($old);
}
...
if ($isAuth) {
    ...
    if (isset($_FILES['dashboard_file']) && $_FILES['dashboard_file']['error'] === UPLOAD_ERR_OK) {
        $newVer  = trim($_POST['dash_version'] ?? '');
        $newNote = trim($_POST['dash_note'] ?? '');

        if (empty($newVer)) {
            $message = 'Nomor versi dashboard tidak boleh kosong!'; $msgType = 'error';
        } elseif (pathinfo($_FILES['dashboard_file']['name'], PATHINFO_EXTENSION) !== 'php') {
            $message = 'File harus berformat .php!'; $msgType = 'error';
        } else {
            $tmpPath  = $_FILES['dashboard_file']['tmp_name'];
            $tmpSize  = filesize($tmpPath);

            // Validasi minimal: file harus mengandung tag PHP dan string kunci
            $tmpContent = file_get_contents($tmpPath);
            if (!str_contains($tmpContent, '<?php') || !str_contains($tmpContent, 'DASHBOARD_PASS')) {
                $message = '✗ File tidak valid — bukan file dashboard yang benar!'; $msgType = 'error';
            } elseif ($tmpSize < 10240) { // minimal 10KB
                $message = '✗ File terlalu kecil, kemungkinan tidak lengkap (min 10KB)!'; $msgType = 'error';
            } else {
                // Buat direktori backup jika belum ada
                if (!is_dir(SELF_BACKUP_DIR)) mkdir(SELF_BACKUP_DIR, 0755, true);

                // Backup file sekarang sebelum di-replace
                $backupName = SELF_BACKUP_DIR . 'dashboard_' . date('Ymd_His') . '_' . getDashVersion()['version'] . '.php';
                if (!copy(SELF_FILE, $backupName)) {
                    $message = '✗ Gagal membuat backup! Update dibatalkan demi keamanan.'; $msgType = 'error';
                } else {
                    // Atomik: tulis ke file sementara dulu, lalu rename
                    $tempDest = SELF_FILE . '.tmp_' . uniqid();
                    if (move_uploaded_file($tmpPath, $tempDest)) {
                        if (rename($tempDest, SELF_FILE)) {
                            // Simpan info versi baru
                            file_put_contents(DASH_VER_FILE, json_encode([
                                'version'    => $newVer,
                                'updated_at' => date('Y-m-d H:i:s'),
                                'note'       => $newNote,
                                'backup'     => basename($backupName),
                                'size_kb'    => round($tmpSize / 1024, 1),
                            ], JSON_PRETTY_PRINT));
                            pruneBackups();
                            writeLog("SELF-UPDATE: dashboard $newVer (" . round($tmpSize/1024,1) . "KB) - $newNote", 'UPDATE');
                            // Redirect agar browser load versi baru
                            header('Location: ' . $_SERVER['PHP_SELF'] . '?updated=1');
                            exit;
                        } else {
                            @unlink($tempDest);
                            $message = '✗ Gagal mengganti file (rename gagal). Cek permission!'; $msgType = 'error';
                        }
                    } else {
                        $message = '✗ Gagal memindahkan file upload. Cek permission!'; $msgType = 'error';
                    }
                }
            }
        }
    ...
    if (isset($_POST['rollback_file'])) {
        $backupFile = SELF_BACKUP_DIR . basename($_POST['rollback_file']); // basename cegah path traversal
        if (!file_exists($backupFile)) {
            $message = '✗ File backup tidak ditemukan!'; $msgType = 'error';
        } else {
            // Backup versi sekarang sebelum rollback
            if (!is_dir(SELF_BACKUP_DIR)) mkdir(SELF_BACKUP_DIR, 0755, true);
            $rollbackBackup = SELF_BACKUP_DIR . 'dashboard_' . date('Ymd_His') . '_before_rollback.php';
            copy(SELF_FILE, $rollbackBackup);

            if (copy($backupFile, SELF_FILE)) {
                writeLog("ROLLBACK: kembali ke " . basename($backupFile), 'ROLLBACK');
                // Reset versi info
                $dashVer = getDashVersion();
                file_put_contents(DASH_VER_FILE, json_encode([
                    'version'    => 'rollback-' . date('Ymd_His'),
                    'updated_at' => date('Y-m-d H:i:s'),
                    'note'       => 'Rollback dari: ' . basename($backupFile),
                    'backup'     => basename($rollbackBackup),
                ], JSON_PRETTY_PRINT));
                header('Location: ' . $_SERVER['PHP_SELF'] . '?rolled_back=1');
                exit;
            } else {
                $message = '✗ Rollback gagal. Cek permission file!'; $msgType = 'error';
            }
        }
    }
    ...
    if (isset($_POST['delete_backup'])) {
        $f = SELF_BACKUP_DIR . basename($_POST['delete_backup']);
        if (file_exists($f)) { unlink($f); $message = '✓ Backup dihapus'; $msgType = 'success'; }
        else { $message = '✗ File backup tidak ada'; $msgType = 'error'; }
    }
}
...
if (isset($_GET['updated']))     { $message = '✓ Dashboard berhasil diupdate ke versi ' . htmlspecialchars($dashVer['version']) . '! Halaman sudah memuat versi baru.'; $msgType = 'success'; }
if (isset($_GET['rolled_back'])) { $message = '↩ Rollback berhasil! Dashboard kembali ke versi sebelumnya.'; $msgType = 'success'; }
?>
<!DOCTYPE html>
<html lang="id">
<head>
...
.ll-update{color:#ff9d00}
.ll-rollback{color:#ff6ec7}

/* ──── SELF UPDATE ──── */
.update-warning{
  background:#ffb70012;border:1px solid #ffb70030;
...
.backup-name{font-family:var(--mono);font-size:11px;color:var(--text2);word-break:break-all}
.backup-meta{font-family:var(--mono);font-size:10px;color:var(--text3);margin-top:2px}
.backup-actions{display:flex;gap:8px;flex-shrink:0}
.diff-badge{
  display:inline-block;padding:2px 8px;border-radius:3px;
  font-size:10px;font-family:var(--mono);letter-spacing:.5px;
}
.diff-self{background:#00e5ff15;color:var(--cyan);border:1px solid #00e5ff30}
.diff-backup{background:#a855f715;color:var(--purple);border:1px solid #a855f730}
...
<div class="main">
  ...
  <div class="stats-row">
    ...
    <div class="stat" style="border-top:2px solid #ff9d00">
      <span class="stat-ic">🔄</span>
      <div class="stat-lbl">Versi Dashboard</div>
      <div class="stat-val" style="color:#ff9d00;font-size:15px"><?= htmlspecialchars($dashVer['version']) ?></div>
      <div class="stat-meta"><?= htmlspecialchars(substr($dashVer['updated_at'], 0, 10)) ?></div>
    </div>
  </div>
  ...
  <div class="tabs">
    ...
    <button class="tab-btn" onclick="switchTab('selfupdate')">🔄 Update Dashboard</button>
  ...
  <!-- ── TAB: SELF UPDATE ── -->
  <div class="tab-pane" id="tab-selfupdate">
    <div class="g2">
      ...
      <div class="card">
        <div class="card-h">
          <div class="card-h-left">
            <div class="card-ic ic-amber">🔄</div>
            <div class="card-title">Upload Versi Baru Dashboard</div>
          </div>
          <span class="badge b-amber"><?= htmlspecialchars($dashVer['version']) ?></span>
        </div>
        <div class="card-b">
          <div class="update-warning">
            ⚠ <div>
              File PHP baru akan <b>langsung menggantikan</b> dashboard yang sedang berjalan.<br>
              Backup otomatis dibuat sebelum penggantian. Pastikan file sudah ditest sebelum upload.
            </div>
          </div>
          <form method="POST" enctype="multipart/form-data"
                onsubmit="return confirm('PERHATIAN!\n\nFile PHP baru akan menggantikan dashboard yang sekarang.\nBackup otomatis akan dibuat.\n\nLanjutkan update?')">
            <div class="mb">
              <label style="font-size:10px;font-family:var(--mono);letter-spacing:1.5px;color:var(--text2);text-transform:uppercase;display:block;margin-bottom:6px">Nomor Versi Baru</label>
              <input class="inp" type="text" name="dash_version" placeholder="v2.0.0" required>
            </div>
            <div class="mb">
              <label style="font-size:10px;font-family:var(--mono);letter-spacing:1.5px;color:var(--text2);text-transform:uppercase;display:block;margin-bottom:6px">Catatan Perubahan</label>
              <input class="inp" type="text" name="dash_note" placeholder="Tambah fitur X, perbaikan bug Y">
            </div>
            <div class="mb">
              <label style="font-size:10px;font-family:var(--mono);letter-spacing:1.5px;color:var(--text2);text-transform:uppercase;display:block;margin-bottom:6px">File Dashboard (.php)</label>
              <input class="inp-file" type="file" name="dashboard_file" accept=".php" required>
            </div>
            <div style="background:var(--bg2);border:1px solid var(--border);border-radius:8px;padding:12px 14px;margin-bottom:16px">
              <div style="font-size:10px;font-family:var(--mono);color:var(--text3);letter-spacing:1px;text-transform:uppercase;margin-bottom:8px">Validasi Otomatis</div>
              <div style="font-family:var(--mono);font-size:11px;color:var(--text2);line-height:2">
                ✓ Ekstensi harus <span style="color:var(--cyan)">.php</span><br>
                ✓ Ukuran minimum <span style="color:var(--cyan)">10 KB</span><br>
                ✓ Harus mengandung <span style="color:var(--cyan)">&lt;?php</span> dan <span style="color:var(--cyan)">DASHBOARD_PASS</span><br>
                ✓ Backup otomatis sebelum replace
              </div>
            </div>
            <button class="btn btn-amber" type="submit" style="width:100%;justify-content:center">
              🔄 UPDATE DASHBOARD SEKARANG
            </button>
          </form>
        </div>
      </div>
      ...
      <div class="card">
        <div class="card-h">
          <div class="card-h-left">
            <div class="card-ic ic-green">ℹ</div>
            <div class="card-title">Versi Dashboard Aktif</div>
          </div>
        </div>
        <div class="card-b">
          <div class="inf-row">
            <span class="inf-k">Versi</span>
            <span class="inf-v" style="color:var(--amber)"><?= htmlspecialchars($dashVer['version']) ?></span>
          </div>
          <div class="inf-row">
            <span class="inf-k">Diupdate</span>
            <span class="inf-v" style="font-size:11px"><?= htmlspecialchars($dashVer['updated_at']) ?></span>
          </div>
          <div class="inf-row">
            <span class="inf-k">Catatan</span>
            <span class="inf-v" style="font-size:11px;color:var(--text2)"><?= htmlspecialchars($dashVer['note'] ?? '-') ?></span>
          </div>
          <div class="inf-row">
            <span class="inf-k">Path File</span>
            <span style="font-family:var(--mono);font-size:10px;color:var(--text3);word-break:break-all"><?= htmlspecialchars(SELF_FILE) ?></span>
          </div>
          <div class="inf-row">
            <span class="inf-k">Ukuran File Aktif</span>
            <span class="inf-v"><?= round(filesize(SELF_FILE)/1024, 1) ?> KB</span>
          </div>
          <div class="inf-row">
            <span class="inf-k">Terakhir Dimodifikasi</span>
            <span class="inf-v" style="font-size:11px"><?= date('d M Y H:i:s', filemtime(SELF_FILE)) ?></span>
          </div>
          <div class="inf-row">
            <span class="inf-k">Backup Dir</span>
            <span class="badge <?= is_dir(SELF_BACKUP_DIR) ? 'b-ok' : 'b-amber' ?>">
              <?= is_dir(SELF_BACKUP_DIR) ? 'ADA' : 'BELUM DIBUAT' ?>
            </span>
          </div>
          <div class="inf-row">
            <span class="inf-k">Jumlah Backup</span>
            <span class="inf-v" style="color:var(--purple)"><?= count($backups) ?> file</span>
          </div>
          <div class="inf-row">
            <span class="inf-k">Writable</span>
            <span class="badge <?= is_writable(SELF_FILE) ? 'b-ok' : 'b-err' ?>">
              <?= is_writable(SELF_FILE) ? 'YA' : 'TIDAK — cek chmod' ?>
            </span>
          </div>
        </div>
      </div>

      <!-- Daftar Backup & Rollback -->
      <div class="card full">
        <div class="card-h">
          <div class="card-h-left">
            <div class="card-ic ic-purple">↩</div>
            <div class="card-title">Backup History & Rollback</div>
          </div>
          <span class="badge b-amber"><?= count($backups) ?> backup tersimpan</span>
        </div>
        <div class="card-b">
          <?php if (empty($backups)): ?>
            <div class="empty-state">
              <div class="empty-icon">📂</div>
              <div>Belum ada backup.<br>Backup dibuat otomatis saat pertama kali melakukan update.</div>
            </div>
          <?php else: ?>
            <div style="margin-bottom:10px;font-family:var(--mono);font-size:11px;color:var(--text3)">
              Klik <b style="color:var(--purple)">ROLLBACK</b> untuk mengembalikan dashboard ke versi backup tersebut.
              File aktif saat ini akan dibackup otomatis sebelum rollback.
            </div>
            <?php foreach ($backups as $i => $bf): ?>
              <?php
                $bName = basename($bf);
                $bSize = round(filesize($bf)/1024, 1);
                $bDate = date('d M Y H:i:s', filemtime($bf));
                $isCurrent = ($i === 0);
              ?>
              <div class="backup-row">
                <div style="flex:1;min-width:0">
                  <div class="backup-name">
                    <?php if ($isCurrent): ?>
                      <span class="diff-badge diff-backup">TERBARU</span>
                    <?php endif; ?>
                    <?= htmlspecialchars($bName) ?>
                  </div>
                  <div class="backup-meta"><?= $bDate ?> · <?= $bSize ?> KB</div>
                </div>
                <div class="backup-actions">
                  <form method="POST"
                        onsubmit="return confirm('Rollback ke <?= htmlspecialchars($bName) ?>?\n\nDashboard saat ini akan dibackup dulu sebelum diganti.')">
                    <input type="hidden" name="rollback_file" value="<?= htmlspecialchars($bName) ?>">
                    <button class="btn btn-purple btn-sm" type="submit">↩ ROLLBACK</button>
                  </form>
                  <form method="POST"
                        onsubmit="return confirm('Hapus backup ini secara permanen?')">
                    <input type="hidden" name="delete_backup" value="<?= htmlspecialchars($bName) ?>">
          ...
          <?php else: ?>
            <?php foreach ($logs as $line): ?>
              <?php
                ...
                elseif (str_contains($line, 'UPDATE') || str_contains($line, 'SELF')) $cls .= ' ll-update';
                elseif (str_contains($line, 'ROLLBACK')) $cls .= ' ll-rollback';
              ?>
